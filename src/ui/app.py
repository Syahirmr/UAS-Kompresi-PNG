...
from src.utils.config import (
    ...
    WINDOW_GEOMETRY, BG_PRIMARY, BG_SECONDARY, COMPARISON_ETA_WINDOW
)
...
class ExportSuccessDialog(ctk.CTkToplevel):
    """Custom popup window for export success notification."""
    def __init__(self, parent, file_path):
        super().__init__(parent)
        self.file_path = file_path
        self.title("Export Berhasil")
        self.geometry("450x200")
        self.resizable(False, False)
        
        # Keep on top and make it modal
        ...
        success_label = ctk.CTkLabel(
            ...
            font=ctk.CTkFont(name="Segoe UI", size=18, bold=True),
            text_color="#107C10"  # Sleek green color
        )
        success_label.pack(pady=(25, 10))
        
        # File info label
        filename_label = ctk.CTkLabel(
            self,
            text=f"Laporan berhasil disimpan ke:\n{Path(file_path).name}",
            font=ctk.CTkFont(name="Segoe UI", size=12),
            wraplength=400
        )
        filename_label.pack(pady=(0, 25))
        ...
        self.open_folder_btn.pack(side=tk.LEFT, padx=(50, 10), expand=True)
        ...
        self.ok_btn.pack(side=tk.RIGHT, padx=(10, 50), expand=True)
...
class CompressionApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        # Track completed algorithms for preview mapping
    ...
    def _bind_events(self):
        ...
        self.control_panel.compress_btn.configure(command=self._start_comparison)
        self.control_panel.cancel_btn.configure(command=self._cancel_compression)
        self.metrics.export_btn.configure(command=self._export_report_action)
    ...
    def _browse_dataset_folder(self):
        ...
        if valid and self.files_list:
            # 1. Total size
            total_bytes = sum(f.stat().st_size for f in self.files_list)
            if total_bytes < 1024 * 1024:
                total_sz_str = f"{total_bytes / 1024:.1f} KB"
            else:
                total_sz_str = f"{total_bytes / (1024*1024):.1f} MB"
            ...
            avg_res = f"{avg_w} x {avg_h}" if avg_w and avg_h else "-"
            
            self.folder_picker.set_dataset_stats(len(self.files_list), total_sz_str, "PNG", avg_res)
        else:
            self.folder_picker.set_dataset_stats(0, "-", "PNG", "-")

        # Update detail selector dropdown items
        self.metrics.populate_details_menu(self.files_list)
        
        # Update start button subtext counter
        self.control_panel.set_file_count(len(self.files_list))
    ...
    def _start_comparison(self):
        """Start comparison for only the selected algorithms in a background thread."""
        valid, message = validate_dataset(self.files_list)
        if not valid:
        ...
        selected_algos = self.control_panel.get_selected_algorithms()
        if not selected_algos:
            messagebox.showwarning("Algoritma Belum Dipilih", "Silakan pilih minimal satu algoritma kompresi.")
            return
        ...
        if "zopfli" in selected_algos:
            messagebox.showinfo(
                "Zopfli Warning",
                "Zopfli bisa sangat lambat untuk dataset besar (60s timeout per file).\n\n"
                "Proses akan dijalankan secara berurutan."
            )
        ...
        self.control_panel.set_status("Memulai kompresi...")
        self.metrics_data = []
        self.metrics_summary = summarize_metrics([])
        self.compressed_outputs = {}
        self.completed_algorithms = set()
        ...
        self.logger.compression_started("comparison", len(self.files_list) * len(selected_algos))
    ...
    def _run_comparison_background(self):
        """Run comparison for only the selected algorithms outside the Tkinter main loop."""
        import time
        from src.compression.compressor import compress_file
        from src.analysis.analyzer import build_metric, summarize_metrics
        from src.compression.comparison_runner import _compute_winners, _compute_overall_summary, _build_comparison_done_text, ALGORITHM_LABELS, _format_eta
        ...
        if total_files == 0:
            self._queue_comparison_progress(0, "Tidak ada file PNG untuk diproses.", None)
            return

        self._queue_comparison_progress(
            0,
            f"Kompresi: {total_files} file × {num_algos} algoritma = {total_steps} task",
            None,
        )

        for algo_idx, algo in enumerate(selected_algos):
            if self.cancel_requested:
                cancelled = True
                break

            algo_output_dir = Path("outputs") / "comparison" / algo
            algo_metrics = []
            algo_results = []
            ...
            for idx, file_path in enumerate(files, start=1):
                ...
                self._queue_comparison_progress(pct, progress_text, None)
                
                # Elapsed time
                elapsed_sec = time.time() - started_at
                elapsed_str = f"{int(elapsed_sec//60):02d}:{int(elapsed_sec%60):02d}"

                # Update real-time labels on UI
                remaining_count = total_steps - step
                self.after(0, lambda f=input_file.name, a=ALGORITHM_LABELS[algo], el=elapsed_str, et=eta_str, rem=remaining_count: 
                    self.control_panel.set_realtime_metrics(f, a, el, et, rem)
                )
                ...
                if result.get("success"):
                    self.after(0, lambda a=algo: self.completed_algorithms.add(a))
                metric = build_metric(input_file, algo, result)
                algo_metrics.append(metric)
                all_metrics.append(metric)

                # Update preview card elapsed time
                self.after(0, lambda a=algo, t=metric["time_ms"]: self.preview.set_algorithm_time(a, t))

                pct_after = min(((steps_done + idx) / total_steps) * 100, 100)
                self._queue_comparison_progress(pct_after, progress_text, metric)
        ...
        done_text = _build_comparison_done_text(per_algorithm_results, cancelled)
        self._queue_comparison_progress(100, done_text, None)
    ...
    def _finish_comparison(self, comparison_result):
        ...
        for algo_key, algo_data in per_algorithm.items():
            if any(r.get("success") for r in algo_data.get("results", [])):
                self.completed_algorithms.add(algo_key)
        ...
        if cancelled:
            self.control_panel.set_status(
                f"Proses dibatalkan. {completed_count} selesai, {failed_count} gagal."
            )
        else:
            ...
            self.control_panel.set_status(
                f"Proses selesai! Best: {wr_label} | Fastest: {ws_label} | Balanced: {wb_label}"
            )
    ...
    def _finish_compression(self, summary):
        ...
        if any(r.get("success") for r in summary.get("results", [])):
            self.completed_algorithms.add(algorithm)
    ...
    def _enable_export_if_ready(self):
        ...
        if self.metrics_data:
            self.metrics.export_btn.configure(state="normal")

    def _export_report_action(self):
        """Handle Export Report button click by opening a Save As dialog directly."""
        if not self.metrics_data:
            self.after(0, lambda: self.control_panel.set_status(
                "Export gagal: belum ada metrics untuk diekspor."
            ))
            messagebox.showwarning("Export Gagal", "Belum ada data metrics hasil kompresi untuk diekspor.")
            return

        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        default_filename = f"compression_results_{timestamp}"

        file_path = filedialog.asksaveasfilename(
            parent=self,
            title="Simpan Laporan Hasil Kompresi",
        ...
        try:
            ...
            self.logger.export_success(str(file_path))
            self.after(0, lambda: self.control_panel.set_status(
                f"Export berhasil: {Path(file_path).name}"
            ))
            ...
            messagebox.showerror("Export Gagal", f"Terjadi kesalahan saat mengekspor file:\n{exc}")
    ...
    def _update_preview_current_file(self):
        ...
        # Get all available compressed outputs for this file
        ...
        selected_algos = self.control_panel.get_selected_algorithms()
        self.preview.display_all_images(original_path, outputs, selected_algos, winners, self.completed_algorithms)
